...
export interface Bindings {
  ...
  clarify: (input: { question: string }) => Promise<unknown>;
  search: (input: { query: string; altQueries?: string[] }) => Promise<unknown>;
  rank: (input: RankInput) => Promise<unknown>;
  generate: (input: GenerateInput) => Promise<unknown>;
  read: (input: { threadId: string }) => Promise<unknown>;
  ask: (input: {
    threadId: string;
    question: string;
    conversation: Array<{
...
export const str = (v: unknown): string => {
  if (typeof v !== "string") throw new EvalError("INVALID_PRODUCT_TEXT");
  return v;
};
const list = (v: unknown): unknown[] => {
  if (!Array.isArray(v)) throw new EvalError("INVALID_PRODUCT_LIST");
  return v;
};
const num = (v: unknown): number => {
  if (typeof v !== "number" || !Number.isFinite(v)) throw new EvalError("INVALID_PRODUCT_NUMBER");
  return v;
};
const citation = (v: unknown, sources: Source[]): Citation => {
  ...
  return {
    sourceId: typeof r.answerId === "string" ? r.answerId : (source?.id ?? "unknown"),
    fingerprint,
    quote: str(r.quote),
    url: typeof r.sourceUrl === "string" ? r.sourceUrl : (source?.url ?? "unknown"),
...
export const normalizeArtifact = (raw: unknown, mode: Artifact["mode"]): Artifact => {
  ...
  const sources: Source[] = list(value.timelineStages).map((v) => {
    ...
    if (kind !== "Answer" && kind !== "Article") throw new EvalError("INVALID_SOURCE_KIND");
    return {
      id: str(stage.answerId),
      fingerprint: str(e.fingerprint),
      text: str(e.excerpt),
      url: str(stage.canonicalUrl),
      author: str(stage.authorDisplayName),
      kind,
      questionId: str(stage.questionId),
      title: str(stage.title),
      editTime: num(stage.editTime),
      capturedAt: num(e.capturedAt),
      contentId: str(e.sourceContentId),
    };
  });
...
export const createBridge = (
  ...
  const failure = (value: Record<string, unknown>): Observation => ({
    status: "error",
    code: typeof value.code === "string" ? value.code : "PRODUCT_ERROR",
    retryable: /HTTP_STATUS:(429|5\d\d)|NETWORK_FAILED|TRANSPORT_FAILED|SEARCH_RATE_LIMITED/.test(
      [...bindings.diagnostics, String(value.code)].join(" "),
    ),
    message: typeof value.message === "string" ? value.message : "Product operation failed",
  });
  return {
    decide: makePolicy(complete),
    invoke: (action, view, runtime) =>
      network.scope(
        runtime,
        "product",
        async () => {
          ...
          if (!seeded) {
            ...
            seeded = true;
            if (view.input.artifact) modes.set(view.input.artifact.id, view.input.artifact.mode);
          }
          ...
          if (action.tool === "search") {
            ...
            if (r.status !== "ok") return failure(r);
            ...
            for (const v of list(r.candidates)) {
              ...
              sources.push({
                id: str(c.answerId),
                questionId: str(c.questionId),
                fingerprint: str(e.fingerprint),
                text: str(e.excerpt),
                url: str(c.url),
                author: typeof c.authorDisplayName === "string" ? c.authorDisplayName : "知乎用户",
                kind,
                title: str(c.title),
                editTime: typeof c.editAt === "number" ? c.editAt : 0,
                capturedAt: num(e.capturedAt),
                contentId: str(e.sourceContentId),
              });
            }
          ...
          if (action.tool === "rank") {
            const r = obj(
              await bindings.rank({
                question,
                refinedQuery: refinedQuery || question,
                learningIntent: intent || view.input.learningTask || question,
                candidates: view.sources.map((s) => ({
                  answerId: s.id,
                  title: s.title || question,
                  authorDisplayName: s.author ?? "知乎用户",
                  preview: s.text.slice(0, 200),
                })),
              }),
            );
            if (r.success !== true) return failure(r);
            ...
            rankings.sort(
              (a, b) =>
                Number(priority.includes(str(b.role))) - Number(priority.includes(str(a.role))),
            );
            return {
              status: "ok",
              rankedIds: rankings.map((r) => str(r.answerId)),
              units: [
                {
                  id: "ranking-summary",
                  kind: "guidance",
          ...
          if (action.tool === "generate") {
            const selected = (action.sourceIds ?? []).map((id) =>
              view.sources.find((s) => s.id === id)!,
            );
            const r = obj(
              await bindings.generate({
                ...
                selectedCandidates: selected.map((s) => ({
                  ...
                  canonicalUrl: s.url,
                  excerptFingerprint: s.fingerprint,
                })),
              }),
            );
            if (r.success !== true) return failure(r);
            ...
            if (loaded.success !== true) return failure(loaded);
            ...
            return r.success === true
              ? {
                  ...
                  artifact: normalizeArtifact(
                    r.artifact,
                    modes.get(view.artifact!.id) ?? "evidence_only",
                  ),
                }
              : failure(r);
          }
          if (action.tool === "ask") {
            const r = obj(
              await bindings.ask({
                threadId: view.artifact?.id ?? "",
                question: query,
                conversation: view.turns.flatMap((t) => [
            ...
            return {
              status: "ok",
              answer: {
                status: a.status === "grounded" ? "answered" : "evidence_gap",
                text: str(a.answer),
                citations: list(a.evidenceRefs).map((ref) =>
                  citation(ref, view.artifact?.sources ?? []),
                ),
                nextQueries: list(a.nextActions)
                  .map(obj)
                  .filter((a) => a.type === "search_supplement" && typeof a.query === "string")
                  .map((a) => str(a.query)),
              },
            };
          }
