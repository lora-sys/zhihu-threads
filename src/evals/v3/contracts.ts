...
import { compare, hash, makeReport, markdown, redact, runCase, selectTasks, validateTask } from "./runner.ts";
...
const test = (name: string, run: () => Promise<void> | void, paths = ["src/evals/v3/"]) => contracts.push({ id: `reg-contract-${contracts.length + 1}`, name, run, affectedPaths: paths });
...
const judge: Judge = async (input, runtime) => {
  ...
  return { taskComplete: true, reason: "Synthetic assessment for contract testing only", units: input.units.map((unit) => ({ id: unit.id, supported: true, relevant: true })) };
};
const measure = { scorer: "test-v3", datasetHash: "test", caseSetHash: "test", fixtureHash: "test", judge: "none", policy: "fixture", budgets: "test", sourceMode: "fixed" as const };
const reportFor = async (task = taskOf()) => makeReport({ complete: true, runId: "test", subject: { commit: "test", model: "none", profile: "fixture" }, measurement: measure, cases: [await runCase(task, { createPort: async () => fixturePort() })], wallMs: 1 });
test("expected labels cannot alter actions or selection", async () => {
  ...
  assert.deepEqual(x.steps.map((s) => [s.action, s.observation]), y.steps.map((s) => [s.action, s.observation]));
...
test("real product refusal stops before search", async () => {
  ...
  assert.deepEqual(x.steps.map((s) => s.action.tool), ["clarify"]);
...
test("long input is forwarded without evaluator truncation", async () => {
  ...
  let seen = "";
  await execute(input, "workflow", fixturePort({ override: (_a, v) => { seen = v.input.question; return { status: "refused", message: "Too long" }; } }), context());
  assert.equal(seen.length, 501);
});
test("blank input reaches the product boundary", async () => {
  let seen = false;
  await execute({ question: "" }, "workflow", fixturePort({ override: () => { seen = true; return { status: "error", code: "INVALID_REQUEST", retryable: false }; } }), context());
  assert.equal(seen, true);
});
for (const task of scenarios().filter((t) => t.mode === "single_step"))
  test(`single-step ${task.input.step!.tool} invokes exactly one module`, async () => {
    const x = await exec(task);
    assert.equal(x.steps.length, 1);
    assert.equal(x.steps[0].action.tool, task.input.step!.tool);
    assert.equal(rules(task, x).rules, "pass");
  }, task.regression!.affectedPaths);
test("missing single-step setup is rejected", () => { const t = taskOf(); t.mode = "single_step"; assert.throws(() => validateTask(t)); });
...
test("semantic judge receives original goal and all questions", async () => {
  const t = scenarios().find((t) => t.mode === "multi_turn")!;
  await grade(t, await exec(t), async (input, runtime) => { assert.equal(input.learningTask, t.input.learningTask); assert.deepEqual(input.units.filter((u) => u.id.startsWith("turn:")).map((u) => u.question), t.input.followUps); return judge(input, runtime); }, context());
});
test("earlier unsupported answer fails the whole semantic grade", async () => {
  const t = scenarios().find((t) => t.mode === "multi_turn")!;
  const g = await grade(t, await exec(t), async (input) => ({ taskComplete: true, reason: "fixture", units: input.units.map((u) => ({ id: u.id, supported: u.id !== "turn:0", relevant: true })) }), context());
...
test("judge failure cannot become a success", async () => {
  const g = await grade(taskOf(), await exec(), async () => { throw Error("offline"); }, context());
  assert.equal(g.semantics, "unavailable");
  assert.equal(g.verdict, "unjudged");
});
test("judge missing units is invalid", async () => {
  const g = await grade(taskOf(), await exec(), async () => ({ taskComplete: true, reason: "fixture", units: [] }), context());
  assert.equal(g.semantics, "invalid");
  assert.equal(g.verdict, "unjudged");
});
test("duplicate semantic ids are invalid", async () => {
  const units = unitsOf(await exec());
  assert.equal(parseAssessment({ taskComplete: true, reason: "fixture", units: units.map(() => ({ id: units[0].id, supported: true, relevant: true })) }, units), undefined);
});
test("unknown semantic support is not zero fabrication", async () => {
  const g = await grade(taskOf(), await exec(), async (input) => ({ taskComplete: true, reason: "unknown", units: input.units.map((u) => ({ id: u.id, supported: null, relevant: true })) }), context());
  assert.equal(g.verdict, "unjudged");
...
test("an undisplayed author is not counted as verified", async () => {
  ...
  for (const step of x.steps)
    for (const unit of step.observation.artifact?.units ?? [])
      for (const ref of unit.citations)
        delete ref.author;
  const g = rules(taskOf(), x);
  assert.equal(g.metrics.authorChecks, 0);
  assert.equal(g.metrics.sourceBinding, true);
});
test("fabricated quotations fail exact containment", async () => { const x = await exec(); x.turns[0].answer.citations[0].quote = "not in excerpt"; assert.equal(rules(taskOf(), x).metrics.quoteContainment, false); });
test("a model cannot forge its own evidence source", async () => {
  ...
  assert.equal(rules(taskOf(), x).checks.some((c) => c.id.startsWith("source_snapshot:") && !c.pass), true);
});
...
test("generated evidence must belong to the actual selection", async () => {
  ...
  assert.equal(rules(taskOf(), x).checks.find((c) => c.id.startsWith("selected_membership:"))!.pass, false);
});
test("valid quotes do not guarantee a supported conclusion", async () => {
  const g = await grade(taskOf(), await exec(), async (input) => ({ taskComplete: true, reason: "overbroad claim", units: input.units.map((u) => ({ id: u.id, supported: false, relevant: true })) }), context());
  assert.equal(g.metrics.quoteContainment, true);
  assert.equal(g.metrics.sourceBinding, true);
  assert.equal(g.verdict, "fail");
});
test("truthful content can still fail the original task", async () => {
  const g = await grade(taskOf(), await exec(), async (input) => ({ taskComplete: false, reason: "task incomplete", units: input.units.map((u) => ({ id: u.id, supported: true, relevant: true })) }), context());
...
test("over-refusal is separate from unsupported-content rate", async () => {
  const x = await exec();
  x.turns[0].answer = { status: "evidence_gap", text: "Insufficient evidence", citations: [], nextQueries: ["query"] };
  const g = rules(taskOf(), x);
...
test("user input containing forbidden text is not an output violation", async () => {
  ...
  assert.equal(rules(t, await exec(t)).checks.find((c) => c.id.startsWith("forbidden_text:"))!.pass, true);
});
test("configured secret in output blocks grading before forwarding it", async () => {
  const x = await exec();
  x.turns[0].answer.text = "secret-value";
  let invoked = false;
  const g = await grade(taskOf(), x, async () => { invoked = true; return {}; }, context(), ["secret-value"]);
  assert.equal(g.verdict, "fail");
...
test("retry recovery retains both attempts and their full usage", async () => {
  let environments = 0;
  const r = await runCase(taskOf(), { judge, createPort: async (_r, n) => { environments++; return fixturePort({ override: a => n === 1 && a.tool === "search" ? { status: "error", code: "HTTP_503", retryable: true } : undefined }); } });
  assert.equal(environments, 2);
  assert.equal(r.firstAttemptSuccess, false);
  assert.equal(r.retrySuccess, true);
  assert.equal(r.attempts.length, 2);
  assert.equal(r.totals.calls, r.attempts.reduce((sum, a) => sum + a.calls.length, 0));
...
test("search requests are not unknown model token requests", () => {
  const ledger = createLedger(5);
  ledger.runtime(new AbortController().signal).beginCall("product", "search", "fixture")({ status: "ok", usage: { costUsd: 0 } });
...
test("deadline bounds a non-resolving tool", async () => {
  const r = await runCase(taskOf(), { executionTimeoutMs: 5, createPort: async () => ({ invoke: async () => new Promise(() => { }) }) });
...
test("deadline bounds a non-resolving judge without false success", async () => {
  const r = await runCase(taskOf(), { judgeTimeoutMs: 5, createPort: async () => fixturePort(), judge: async () => new Promise(() => { }) });
...
test("autonomous policy sees observations but not expected", async () => {
  const t = taskOf();
  t.mode = "autonomous";
  const r = await runCase(t, { createPort: async () => fixturePort({ policy: async (v) => { assert.equal("expected" in v, false); assert.equal("expected" in v.input, false); return fixturePolicy(v); } }) });
  assert.equal(r.firstAttemptRulesPass, true);
});
test("autonomous tool failure can be recovered within the same attempt", async () => {
  ...
  const t = taskOf();
  t.mode = "autonomous";
  const r = await runCase(t, { createPort: async () => fixturePort({ override: a => { if (a.tool === "search" && !failed) {
        failed = true;
        return { status: "error", code: "HTTP_503", retryable: true };
      } }, policy: async (v) => v.observations.at(-1)?.observation.status === "error" ? { tool: "search", query: "retry query" } : fixturePolicy(v) }) });
  assert.equal(r.firstAttemptRulesPass, true);
  assert.equal(r.attempts.length, 1);
...
test("autonomous endless policy is stopped by step budget", async () => {
  const t = taskOf();
  t.mode = "autonomous";
  const r = await runCase(t, { maxSteps: 3, createPort: async () => fixturePort({ policy: async () => ({ tool: "search", query: "query" }) }) });
  assert.equal(r.attempts[0].execution.outcome, "budget_exceeded");
...
test("premature finish is rejected", async () => {
  const t = taskOf();
  t.mode = "autonomous";
  const r = await runCase(t, { createPort: async () => fixturePort({ policy: async () => ({ tool: "finish" }) }) });
  assert.equal(r.firstAttemptRulesPass, false);
});
test("unknown tools are rejected", () => assert.throws(() => parseAction({ tool: "shell", query: "echo" })));
test("unknown selected sources are rejected before generation", async () => {
  ...
  assert.equal(x.steps.some((s) => s.action.tool === "generate"), false);
});
test("model policy adapter parses actual observations and does not receive labels", async () => {
  let modelCalls = 0;
  const p = makePolicy(async (messages) => { modelCalls++; const view = JSON.parse(messages[1].content); assert.equal("expected" in view, false); return JSON.stringify(await fixturePolicy(view)); });
  const t = taskOf();
  t.mode = "autonomous";
...
test("offset skips tasks independently of stride", () => { const all = scenarios(); assert.equal(selectTasks(all, { offset: 2, limit: 1 })[0].id, all[2].id); });
test("unknown task id is not silently ignored", () => assert.throws(() => selectTasks(scenarios(), { ids: ["missing"] })));
...
test("real regressions must name contract and affected paths", () => { const t = taskOf(); t.purpose = "regression"; assert.throws(() => validateTask(t)); });
...
test("offline network rejects real requests before fetch", async () => {
  let requests = 0;
  const n = installNetwork({ live: false, origins: ["https://example.invalid"], maxRunCalls: 10, fetcher: async () => { requests++; return Response.json({}); } });
  try {
...
test("unscoped and off-origin requests are denied", async () => {
  ...
  try {
    ...
    await assert.rejects(n.scope(context(), "product", () => fetch("https://other.invalid"), async () => Response.json({})));
  }
  finally {
    n.restore();
  }
...
test("network meters every response including token usage and phase", async () => {
  ...
  const n = installNetwork({ live: false, origins: ["https://example.invalid"], maxRunCalls: 10 });
  try {
    await n.scope(ledger.runtime(new AbortController().signal), "judge", () => fetch("https://example.invalid/chat/completions"), async (_url, init) => { assert.equal(init?.redirect, "error"); return Response.json({ usage: { prompt_tokens: 9, completion_tokens: 4 } }); });
    const calls = ledger.close();
...
test("run network budget counts all attempts", async () => {
  ...
  try {
    await n.scope(context(), "product", () => fetch("https://example.invalid"), async () => Response.json({}));
    await assert.rejects(n.scope(context(), "product", () => fetch("https://example.invalid"), async () => Response.json({})));
...
test("credential redaction covers nested values and named secrets", () => {
  const x = redact({ a: [{ body: "secret-value", authorization: "Bearer abc123" }] }, ["secret-value"]);
  assert.equal(JSON.stringify(x).includes("secret-value"), false);
  assert.equal(JSON.stringify(x).includes("abc123"), false);
});
test("hash is stable across object key order", () => assert.equal(hash({ a: 1, b: 2 }), hash({ b: 2, a: 1 })));
test("fixture content is explicitly synthetic", () => assert.ok(artifactOf().sources.every((s) => s.kind === "fixture" && s.url.includes("example.invalid"))));
contracts.push(...bridgeContracts);
test("legacy conversion keeps execution identical when only expected changes", () => {
  const item = { id: "legacy", title: "legacy", category: "adversarial", input: { question: "query" }, expected: { flow: "full" as const } };
  assert.deepEqual(adaptLegacy(item).input, adaptLegacy({ ...item, expected: { flow: "safe_no_thread" } }).input);
});
