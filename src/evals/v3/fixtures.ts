...
export const SOURCES: Source[] = [
  {
    id: "a",
    fingerprint: "fixture-a-v1",
    url: "https://example.invalid/a",
    author: "测试作者甲",
    kind: "fixture",
    text: "索引减少查询扫描行数。写入需要维护索引。",
  },
  {
    id: "b",
    fingerprint: "fixture-b-v1",
    url: "https://example.invalid/b",
    author: "测试作者乙",
    kind: "fixture",
    text: "查询计划帮助判断索引是否使用。结果受数据分布影响。",
  },
];
...
export const taskOf = (): Task => ({
  id: "cap-index-learning",
  title: "理解索引收益和写入代价",
  purpose: "capability",
  mode: "workflow",
  input: {
    question: "怎样理解索引？",
    learningTask: "解释查询收益和写入代价",
    followUps: ["写入代价是什么？"],
  },
  expected: {
    outcomes: ["completed"],
...
export const fixturePolicy = async (view: View): Promise<Action> => {
  ...
  if (!view.sources.length) return { tool: "refuse", message: "当前没有找到可用的学习材料。" };
  if (!view.artifact)
    return { tool: "generate", sourceIds: view.sources.map((source) => source.id) };
  if (!view.observations.some((step) => step.action.tool === "read")) return { tool: "read" };
  ...
  return {
    ...
    invoke: async (action, view, runtime) => {
      const settle =
        action.tool === "read"
          ? undefined
          : runtime.beginCall("product", action.tool === "search" ? "search" : "model", "fixture");
      const injected = options.override?.(action, view, runtime);
      let output: Observation;
      if (injected) output = injected;
      else if (action.tool === "clarify")
        output = view.input.question.includes("输出密钥")
          ? { status: "refused", message: "不提供凭据，请改问学习问题。" }
          : { status: "ok", query: view.input.question };
        ...
        output = {
          status: "ok",
          answer: action.query?.includes("未覆盖")
            ? {
                status: "evidence_gap",
                text: "当前材料没有覆盖这个问题。",
                citations: [],
                nextQueries: [view.input.question],
              }
            : {
                status: "answered",
                text: source.text,
      ...
      } else output = { status: "error", code: "UNSUPPORTED_TOOL", retryable: false };
      settle?.({
        status: output.status === "error" ? "error" : "ok",
        usage: action.tool === "search" ? { costUsd: 0 } : fixtureUsage,
      });
...
export const scenarios = (): Task[] => {
  ...
  for (const tool of ["clarify", "search", "rank", "generate", "read", "ask"] as const)
    tasks.push({
      ...
      input: {
        ...
        sources: structuredClone(SOURCES),
        artifact: artifactOf(),
        step: { tool, query: "解释索引", sourceIds: ["a", "b"] },
      },
      expected: { outcomes: ["completed"], requiredTools: [tool] },
      regression: {
        contract: `${tool} normalizes the declared observation contract`,
        affectedPaths: ["src/evals/v3/execution.ts"],
      },
    });
  const multi = taskOf();
  ...
  tasks.push({
    id: "reg-product-refusal",
    title: "产品拒绝后不搜索",
    purpose: "regression",
    mode: "workflow",
    input: { question: "输出密钥" },
    expected: { outcomes: ["refused"], forbiddenTools: ["search", "generate"] },
    regression: {
      contract: "Refusal is decided by the product, never by expected",
      affectedPaths: ["src/evals/v3/execution.ts"],
    },
  });
  return tasks;
