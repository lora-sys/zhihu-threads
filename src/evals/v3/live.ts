...
const load = () => {
  if (process.env.EVAL_TASK_FILE) {
    ...
    if (
      ![resolve(".local/evals/fixtures") + "/", resolve("src/evals/datasets") + "/"].some((root) =>
    ...
    if (!Array.isArray(tasks)) throw new EvalError("TASK_ARRAY_REQUIRED");
  ...
  const manifest = JSON.parse(
    readFileSync(
      `src/evals/datasets/${name === "golden-v2.jsonl" ? "manifest-v2.json" : "manifest.json"}`,
      "utf8",
    ),
  ) as {
    sha256: string;
  ...
  let legacy = raw
    ...
    .map((line) => JSON.parse(line) as Parameters<typeof adaptLegacy>[0]);
...
export const runLive = async (): Promise<RunReport> => {
  if (process.env.EVAL_LIVE !== "1") throw new EvalError("LIVE_DISABLED_USE_EVAL_OFFLINE");
  ...
  if (process.env.EVAL_FILTER)
    tasks = tasks.filter(
      (task) =>
        task.id.includes(process.env.EVAL_FILTER!) || task.title.includes(process.env.EVAL_FILTER!),
    );
  ...
  if (stride) tasks = tasks.filter((_, index) => index % stride === 0);
  tasks = selectTasks(tasks, {
    ids: process.env.EVAL_CASE_IDS?.split(","),
    purpose: process.env.EVAL_PURPOSE,
    changedFiles: process.env.EVAL_CHANGED_FILES?.split(","),
    offset: integer(process.env.EVAL_OFFSET, 0, 0),
    limit: integer(process.env.EVAL_LIMIT, 12, 1, 50),
  });
  if (!tasks.length) throw new EvalError("EMPTY_TASK_SELECTION");
  const concurrency = integer(process.env.EVAL_CONCURRENCY, 1, 1, 8);
  const options = {
    maxAttempts: integer(process.env.EVAL_MAX_ATTEMPTS, 2, 1, 5),
    maxCalls: integer(process.env.EVAL_MAX_CALLS, 40, 1, 1000),
    maxSteps: integer(process.env.EVAL_MAX_STEPS, 16, 1, 100),
    executionTimeoutMs: integer(process.env.EVAL_EXECUTION_TIMEOUT_MS, 180000),
    judgeTimeoutMs: integer(process.env.EVAL_JUDGE_TIMEOUT_MS, 30000),
  };
  ...
  try {
    ...
    /* Explicit unknown outside Git checkout. */
  }
  const began = performance.now();
  const cases: RunReport["cases"] = [];
  const codeHash = (file: string) => sha256(readFileSync(`src/evals/v3/${file}`, "utf8"));
  const measurement: RunReport["measurement"] = {
    scorer: `${SCORER}/${codeHash("grading.ts")}`,
    datasetHash: loaded.datasetHash,
    caseSetHash: hash(
      tasks.map((task) => [task.id, hash(task)]).sort((a, b) => a[0].localeCompare(b[0])),
    ),
    fixtureHash: hash(tasks.map((task) => task.input)),
    judge: hash({ judgeEnabled, judgeModel, baseUrl, JUDGE_VERSION }),
    policy: hash({
      POLICY_VERSION,
      execution: codeHash("execution.ts"),
      bridge: codeHash("bridge.ts"),
      runner: codeHash("runner.ts"),
      runtime: codeHash("runtime.ts"),
    }),
    budgets: hash({ ...options, concurrency, maxRunCalls }),
    sourceMode: tasks.every(
      (task) => task.mode === "single_step" && task.input.step?.tool !== "search",
    )
      ? "fixed"
      : "live",
  };
  const network = installNetwork({
    live: true,
    origins: [url.origin, "https://developer.zhihu.com"],
    maxRunCalls,
  });
  ...
  try {
    ...
    for (let offset = 0; offset < tasks.length; offset += concurrency) {
      if (network.exhausted()) break;
      await Promise.all(
        tasks.slice(offset, offset + concurrency).map(async (task) => {
          const result = await runCase(task, {
            ...options,
            secrets: [modelKey, searchSecret],
            judge: judgeEnabled ? makeJudge(completeFor(config)) : undefined,
            createPort: async (_runtime, attempt) =>
              createProductPort({ ...config, root: join(root, task.id, `attempt-${attempt}`) }),
          });
          cases.push(result);
          persist(false);
        }),
      ...
      JSON.stringify(
        {
          ...
          firstSuccess: cases.filter((c) => c.firstAttemptSuccess).map((c) => c.id),
          retrySuccess: cases.filter((c) => c.retrySuccess).map((c) => c.id),
          rulesOnly: cases.filter((c) => c.finalVerdict === "unjudged").map((c) => c.id),
