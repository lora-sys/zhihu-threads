...
export const installNetwork = (options: {
  ...
  if (installed) throw new EvalError("NETWORK_SCOPE_ALREADY_INSTALLED");
  ...
  globalThis.fetch = async (input, init) => {
    ...
    if (!scope) throw new EvalError("UNSCOPED_EVAL_NETWORK_REQUEST");
    const url = new URL(input instanceof Request ? input.url : String(input));
    if (!options.origins.includes(url.origin) || url.username || url.password)
      throw new EvalError("NETWORK_ORIGIN_DENIED");
    if (!scope.fixtureFetch && !options.live) throw new EvalError("OFFLINE_NETWORK_DENIED");
    ...
    const settle = scope.runtime.beginCall(
      scope.phase,
      provider,
      scope.fixtureFetch ? "fixture" : "live",
    );
    calls++;
    const signals = [
      scope.runtime.signal,
      init?.signal,
      input instanceof Request ? input.signal : undefined,
    ].filter((s): s is AbortSignal => Boolean(s));
    try {
      const response = await (scope.fixtureFetch ?? fetcher)(input, {
        ...init,
      ...
      if (provider === "model") {
        try {
          ...
          /* Usage absent or malformed stays unknown, not zero. */
        }
      }
      settle({
  ...
  return {
    scope: <T>(
      ...
      operation: () => Promise<T>,
      fixtureFetch?: typeof fetch,
    ): Promise<T> => scopes.run({ runtime, phase, fixtureFetch }, operation),
    calls: () => calls,
