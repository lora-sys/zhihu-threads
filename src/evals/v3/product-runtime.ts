...
export const completeFor =
  (config: ProductConfig): Complete =>
  (messages, runtime, phase) =>
    config.network.scope(
      runtime,
      phase,
      () => {
...
export const createProductPort = async (config: ProductConfig) => {
  ...
  const onError = (error: unknown) => {
    diagnostics.push(error instanceof Error ? error.message : "Product error");
  };
  const chat =
    (timeoutMs: "30 seconds" | "60 seconds" | "90 seconds") => async (key: string, model: string) =>
      makeOpenAiChatCompletions({
        apiKey: key,
        model,
        baseUrl: config.baseUrl,
        transport: makeFetchOpenAiTransport({ timeoutMs }),
        timeoutMs,
      });
  ...
  const bindings: Bindings = {
    ...
    seed: async (input) => {
      ...
      for (const source of sources) {
        ...
        const parsed = createAnswerExcerpt({
          questionId: source.questionId ?? "",
          answerId: source.id,
          capturedAt: source.capturedAt,
          sourceContentId: source.contentId,
          sourceContentType: source.kind,
          sourceEditTime: source.editTime,
          excerpt: source.text,
        });
      ...
      if (input.artifact) {
        const native = createQuestionLearningThread({
          threadId: input.artifact.id,
          question: input.question,
          refinedQuery: input.question,
          createdAt: 1,
          timelineStages: sources.map((s, index) => ({
            questionId: s.questionId ?? "",
            answerId: s.id,
            title: s.title ?? input.question,
            authorDisplayName: s.author ?? "知乎用户",
            editTime: s.editTime!,
            canonicalUrl: s.url,
            excerpt: snapshots[index],
          })),
          learningNodes: input.artifact.units
            .filter((u) => u.kind === "claim")
            .map((u) => ({
              kind: "relationship",
              title: u.id,
              summary: u.text,
              sourceAnswerId: u.primarySource?.id ?? u.citations[0]?.sourceId ?? "",
              sourceUrl: u.primarySource?.url ?? u.citations[0]?.url ?? "",
              evidenceRefs: u.citations.map((r) => ({
                excerptFingerprint: r.fingerprint,
                quote: r.quote,
              })),
              uncertainty: 0.5,
            })),
        ...
        if (native._tag !== "success") throw new EvalError("INVALID_FIXED_THREAD");
    ...
    search: createSearchAnswerCandidatesHandler({
      getSecret: () => config.searchSecret,
      createStore: async () => excerpts,
      createQuotaGuard: async () => quota,
    }),
    rank: createRankAnswerCandidatesHandler({ ...common, createChat: chat("30 seconds"), onError }),
    generate: createGenerateThreadHandler({
      ...common,
      getSecret: () => (config.evidenceOnly ? undefined : config.modelKey),
      createChat: chat("90 seconds"),
      createExcerptStore: async () => excerpts,
      createThreadStore: async () => threads,
      onError,
    }),
    read: createReadThreadHandler({ createThreadStore: async () => threads }),
    ask: createAskThreadAgentHandler({
      ...common,
      createChat: chat("60 seconds"),
      createThreadStore: async () => threads,
    }),
