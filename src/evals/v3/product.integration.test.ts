...
const fixtureFetch: typeof fetch = async (url, init) => {
  if (
    ...
    ).pathname.endsWith("zhihu_search")
  )
  ...
  const value = system.includes("help clarify")
    ? {
        refinedQuery: "索引取舍",
        alternatives: ["索引写入成本"],
        learningIntent: "理解查询和写入的取舍",
        guidance: "选择材料",
        confidence: 0.8,
      }
    : system.includes("search candidate")
      ? {
          summary: "比较材料",
          rankings: ids.map((answerId) => ({ answerId, role: "baseline", reason: "说明索引取舍" })),
          confidence: 0.8,
        }
      : {
          status: "evidence_gap",
          answer: "当前摘录不足。",
          evidenceRefs: [],
          nextActions: [{ type: "search_supplement", label: "补充来源", query: "索引取舍" }],
          uncertainty: 1,
        };
  return Response.json({
...
const run = async (task: Task, fetcher = fixtureFetch) => {
  ...
  const network = installNetwork({
    live: false,
    origins: ["https://model.invalid", "https://developer.zhihu.com"],
    maxRunCalls: 30,
  });
  try {
    return await runCase(task, {
      maxAttempts: 1,
      createPort: async () =>
        createProductPort({
          root,
          ...
          searchSecret: "fixture-search-key",
...
const task = (): Task => ({
  id: "reg-real-handlers",
  title: "真实处理函数和 SQLite 的离线集成",
  purpose: "regression",
  mode: "workflow",
  regression: {
    contract: "Persist and reload a source-backed learning artifact",
    affectedPaths: [
      "src/server/",
      "src/lib/thread-artifact-store.ts",
      "src/evals/v3/product-runtime.ts",
    ],
  },
  input: { question: "如何理解索引？", followUps: ["请总结证据边界"] },
  expected: { outcomes: ["completed"], minSources: 3 },
});
