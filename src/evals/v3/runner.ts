...
export const validateTask = (task: Task): void => {
  ...
  if (
    !["capability", "regression"].includes(task.purpose) ||
    !["single_step", "workflow", "multi_turn", "autonomous"].includes(task.mode)
  )
  ...
  if (
    ...
    (!Array.isArray(input.sources) ||
      ...
      !input.sources.every(validSource))
  )
  ...
  if (task.mode === "single_step" && !input.step) throw new EvalError("SINGLE_STEP_REQUIRED");
  if (task.mode === "multi_turn" && !input.followUps?.length)
    throw new EvalError("FOLLOWUPS_REQUIRED");
  const expected = task.expected;
  if (
    !expected ||
    !strings(expected.outcomes) ||
    !expected.outcomes.length ||
    expected.outcomes.some(
      (x) => !["completed", "refused", "evidence_gap", "error", "budget_exceeded"].includes(x),
    )
  )
    throw new EvalError("INVALID_EXPECTED_OUTCOMES");
  if (expected.minSources !== undefined) integer(expected.minSources, 0, 0, 100);
  if (
    expected.concepts !== undefined &&
    (!Array.isArray(expected.concepts) ||
      expected.concepts.some(
        (group) => !strings(group) || !group.length || group.some((x) => !x.trim()),
      ))
  )
  ...
  if (
    expected.answerability !== undefined &&
    (!strings(expected.answerability) ||
      expected.answerability.length !== input.followUps?.length ||
      expected.answerability.some(
        (x) => !["answerable", "insufficient", "unsafe", "unknown"].includes(x),
      ))
  )
    throw new EvalError("INVALID_ANSWERABILITY");
  if (
    task.purpose === "regression" &&
    (!task.regression?.contract?.trim() ||
      !strings(task.regression.affectedPaths) ||
      !task.regression.affectedPaths.length ||
      task.regression.affectedPaths.some((path) => !path.trim()))
  )
...
export const runCase = async (task: Task, options: Options): Promise<CaseResult> => {
  ...
  for (let number = 1; number <= maxAttempts; number++) {
    ...
    try {
      graded = await grade(
        ...
        execution,
        options.judge,
        ledger.runtime(gradeDeadline.signal),
        options.secrets,
      );
    } finally {
      gradeDeadline.clear();
    ...
    try {
      if (port?.dispose) await bounded(port.dispose(), cleanupDeadline.signal);
    } catch {
    ...
    if (!execution.error?.retryable) break;
  }
  ...
  return {
    ...
    retrySuccess:
      first.grade.verdict !== "pass" &&
      attempts.slice(1).some((attempt) => attempt.grade.verdict === "pass"),
    firstAttemptRulesPass: first.grade.rules === "pass",
    retryRulesPass:
      first.grade.rules !== "pass" &&
      attempts.slice(1).some((attempt) => attempt.grade.rules === "pass"),
    finalVerdict: final.grade.verdict,
    totals: totals(attempts.flatMap((attempt) => attempt.calls)),
    wallMs: performance.now() - began,
  };
};
export const makeReport = (
  input: Omit<RunReport, "schemaVersion" | "totals" | "totalsByPhase">,
): RunReport => {
  ...
  return tasks
    ...
    .filter(
      (task) =>
        ...
        (task.purpose === "regression" &&
          task.regression!.affectedPaths.some((path) =>
            options.changedFiles!.some(
              (file) => file === path || file.startsWith(path.endsWith("/") ? path : `${path}/`),
            ),
          )),
    )
...
export const compare = (before: RunReport, after: RunReport) => {
  ...
  if (before.schemaVersion !== 3 || after.schemaVersion !== 3) reasons.push("schema_version");
  if (!before.complete || !after.complete) reasons.push("incomplete_run");
  if (hash(before.measurement) !== hash(after.measurement))
    reasons.push("measurement_configuration");
  if (before.subject.profile !== after.subject.profile) reasons.push("execution_profile");
  if ([...before.cases, ...after.cases].some((c) => !c.task || hash(c.task) !== c.taskHash))
    reasons.push("task_hash_integrity");
  const identities = (report: RunReport) =>
    report.cases.map((c) => [c.id, c.taskHash]).sort(([a], [b]) => a.localeCompare(b));
  ...
  return {
    ...
    reasons,
    interpretation:
      after.measurement.sourceMode === "fixed"
        ? "controlled-input comparison"
        : "observational only: live retrieval can change",
    regressions: after.cases
      .filter(
        (c) =>
          c.purpose === "regression" &&
          old.get(c.id)!.firstAttemptRulesPass &&
          !c.firstAttemptRulesPass,
      )
      .map((c) => c.id),
    recoveries: after.cases
      .filter(
        (c) =>
          c.purpose === "regression" &&
          !old.get(c.id)!.firstAttemptRulesPass &&
      ...
      .map((c) => c.id),
    semanticRegressions: after.cases
      .filter((c) => old.get(c.id)!.firstAttemptSuccess && c.attempts[0].grade.verdict === "fail")
      .map((c) => c.id),
    unjudgedAfter: after.cases
      .filter((c) => c.attempts[0].grade.verdict === "unjudged")
      .map((c) => c.id),
  };
};
...
export const markdown = (report: RunReport): string => {
  const lines = [
    "# Eval v3 report",
    "",
    `Run ${report.runId}. Complete ${report.complete}. Profile ${report.subject.profile}.`,
  ...
  for (const c of report.cases)
    lines.push(
      `| ${[c.id, c.purpose, c.firstAttemptSuccess, c.retrySuccess, c.firstAttemptRulesPass, c.retryRulesPass, c.finalVerdict, c.totals.calls, c.wallMs.toFixed(2)].map(cell).join(" | ")} |`,
    );
  for (const purpose of ["capability", "regression"] as const) {
    ...
    lines.push(
      "",
      `## ${purpose}`,
      `${cases.length} cases; ${cases.filter((c) => c.firstAttemptSuccess).length} first semantic successes; ${cases.filter((c) => c.retrySuccess).length} recovered semantic successes; ${cases.filter((c) => c.firstAttemptRulesPass).length} first rule passes.`,
    );
  }
  ...
  for (const c of report.cases)
    for (const a of c.attempts)
      lines.push(
        "",
        `### ${c.id} attempt ${a.number}`,
        `Rules ${a.grade.rules}. Semantics ${a.grade.semantics}. Verdict ${a.grade.verdict}.`,
        `Execution ${a.executionMs.toFixed(2)} ms. Grading ${a.gradingMs.toFixed(2)} ms. Wall ${a.wallMs.toFixed(2)} ms.`,
        `Usage ${JSON.stringify(a.totals)}`,
        `Quality ${JSON.stringify(a.grade.metrics)}`,
        ...a.grade.checks
          .filter((check) => !check.pass)
          .map((check) => `Failed ${cell(check.id)}. ${cell(check.detail)}`),
      );
  lines.push(
    "",
    "## Total usage",
    ...
    "",
    "## By phase",
    JSON.stringify(report.totalsByPhase),
    "",
    "Null tokens/cost mean unknown. Known subtotals are not a total bill. Provider durations are summed work, not wall latency.",
  );
  return lines.join("\n") + "\n";
};
  ...
  const visit = (v: unknown): unknown =>
    typeof v === "string"
      ? secrets
          ...
          .reduce((s, secret) => s.split(secret).join("[REDACTED]"), v)
        ...
        : v && typeof v === "object"
          ? Object.fromEntries(
              Object.entries(v).map(([k, x]) => [
                k,
                /^(authorization|api_?key|access_?secret|app_?key)$/i.test(k)
                  ? "[REDACTED]"
                  : visit(x),
              ]),
            )
